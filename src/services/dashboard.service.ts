...
export const dashboardService = {
    ...
    async getStudentStats(studentId: number) {
        const [
            ...
            prisma.studentScholarship.count({ where: { studentId, status: 'Pending' } }),
            prisma.studentScholarship.count({ where: { studentId, status: 'Approved' } }),
            prisma.studentScholarship.findMany({
        ...
        return {
            ...
            approvedApplications,
            totalAwardedAmount,
            availableScholarships: await prisma.scholarship.count({ where: { isActive: true } }),
        };
    },
    ...
    async getRecentActivity(): Promise<RecentActivity> {
        ...
        return {
            ...
            upcomingDeadlines: upcomingDeadlines.filter(
                (s): s is typeof s & { applicationEnd: Date } => s.applicationEnd !== null
            ),
        };
    },
};
