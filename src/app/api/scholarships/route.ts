    ...
    if (action === 'counts') {
      ...
      const [total, internal, external] = await Promise.all([
        ...
        prisma.scholarship.count({
          where: { source: 'INTERNAL', isArchived: includeArchived ? true : false },
        }),
        prisma.scholarship.count({
          where: { source: 'EXTERNAL', isArchived: includeArchived ? true : false },
        }),
      ]);
        ...
        percentSubsidy:
          body.percentSubsidy !== undefined
            ...
            : (() => {
                const totalFees =
                  (body.tuitionFee || 0) +
                  (body.miscellaneousFee || 0) +
                  (body.laboratoryFee || 0) +
                  (body.otherFee || 0);
